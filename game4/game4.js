...
const CONFIG = {
    ...
    ENEMY_VISIBLE_TIME: 8000, // 8 秒可見
    ENEMY_INVISIBLE_TIME: 2000, // 2 秒不可見
    PLAYER_COLOR: '#a0c5db',
    ENEMY_COLOR: '#c34e17',
...
// 生成隨機位置（排除障礙物）
function getRandomPosition(radius, excludePlayer = null) {
    const maxAttempts = 100;
    for (let i = 0; i < maxAttempts; i++) {
        const x = radius + Math.random() * (CONFIG.CANVAS_WIDTH - radius * 2);
        const y = radius + Math.random() * (CONFIG.CANVAS_HEIGHT - radius * 2);
        ...
        // 如果需要避開玩家，檢查是否與玩家太近
        if (excludePlayer) {
            const dx = x - excludePlayer.x;
            const dy = y - excludePlayer.y;
            const distance = Math.sqrt(dx * dx + dy * dy);
            if (distance < 150) continue; // 確保至少距離玩家150像素
        }
...
class Enemy {
    ...
    respawn(player) {
        const newPos = getRandomPosition(this.radius, player);
        this.x = newPos.x;
        this.y = newPos.y;
        this.direction = Math.random() * Math.PI * 2;
    }
...
class Game {
    constructor() {
        ...
        this.elapsedTime = 0;
        this.lastEnemyStateChangeTime = Date.now(); // 保險員狀態切換時間
    ...
    update() {
        ...
        // 處理保險員閃現機制
        const timeSinceStateChange = Date.now() - this.lastEnemyStateChangeTime;

        if (this.enemy.isVisible) {
            // 可見狀態：檢查是否需要切換為不可見
            if (timeSinceStateChange >= CONFIG.ENEMY_VISIBLE_TIME) {
                this.enemy.isVisible = false;
                this.lastEnemyStateChangeTime = Date.now();
            }
        } else {
            // 不可見狀態：檢查是否需要切換為可見
            if (timeSinceStateChange >= CONFIG.ENEMY_INVISIBLE_TIME) {
                this.enemy.isVisible = true;
                this.enemy.respawn(this.player); // 隨機重新定位
                this.lastEnemyStateChangeTime = Date.now();
            }
        }

        // 檢查碰撞（只在保險員可見時）
        if (this.enemy.isVisible && circleCircleCollision(
